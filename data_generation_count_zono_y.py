...
def main():
    num_groups = 1
    zono_distribution = [0, 1] # 0% numbers, 100% zonotopes
    coeff_range = [-100, 100]
    center_range = [-50, 50]
    relation_degree = 1 # 0% zonotopes in a group are related (shared error terms)
    max_total_unknowns = None
    size = 1000
    num_variables = [1, 2, 3] # number of variables: 1, 2, 3
    var_distribution = [0.5, 0.35, 0.15] # corresponding to num_variables. Probability: 80%, 15%, 5%
    shared_errors = 500
    seed = 34
    ...
    common_params["output_file"] = f'count_y_{num_groups}_groups_size_{size}_relation_degree_{int(relation_degree * 100)}.parquet'
